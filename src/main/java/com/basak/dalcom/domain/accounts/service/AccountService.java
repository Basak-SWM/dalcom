...
public class AccountService {
     ...
     * @throws DuplicatedFieldException: 중복된 전화번호 또는 이메일로 요청된 경우
     */
    @Transactional
    public Account userSignUp(UserSignupDto requestDto) {
        try {
            ...
            userProfileService.createUserProfile(account, requestDto.getVoiceUsageAgreement());

            return account;
        } catch (DataIntegrityViolationException ex) {
            if (ex.getMostSpecificCause().getMessage().contains("Duplicate entry")) {
                throw new DuplicatedFieldException("Email or Phone Number");
            }
